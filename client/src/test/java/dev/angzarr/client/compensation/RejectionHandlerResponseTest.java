 ...
 *
 * <p>Tests the unified response type for rejection handlers that can return both compensation
 * events AND upstream notification.
 */
class RejectionHandlerResponseTest {
  ...
  private static EventBook makeEventBook() {
    return EventBook.newBuilder()
        .addPages(
            EventPage.newBuilder()
                .setEvent(Any.newBuilder().setTypeUrl("type.googleapis.com/test.TestEvent").build())
                .build())
        .build();
  }
  ...
  private static Notification makeNotification(String domain, String commandType, String reason) {
    ...
    return Notification.newBuilder().setPayload(Any.pack(rejection)).build();
  }
}
