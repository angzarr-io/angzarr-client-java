 ...
 *
 * <p>Verifies the response type from process manager handlers that can contain both commands (to
 * send to other aggregates) and process events (to persist PM state).
 */
class ProcessManagerResponseTest {
  ...
  void response_commands_are_accessible() {
    List<CommandBook> commands =
        List.of(
            makeCommandBook("order", "CreateOrder"), makeCommandBook("inventory", "ReserveStock"));
    ...
    return CommandBook.newBuilder()
        ...
        .addPages(
            CommandPage.newBuilder()
                .setCommand(
                    Any.newBuilder().setTypeUrl("type.googleapis.com/test." + commandType).build())
                .build())
        .build();
  }
    ...
    return EventBook.newBuilder()
        ...
        .addPages(
            EventPage.newBuilder()
                .setEvent(
                    Any.newBuilder().setTypeUrl("type.googleapis.com/test." + eventType).build())
                .build())
        .build();
  }
}
