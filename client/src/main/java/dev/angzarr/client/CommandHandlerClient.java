 ...
 *
 * <p>CommandHandlerClient handles command routing, response parsing, and provides multiple
 * execution modes:
 *
 * <ul>
 *   <li>Async (fire-and-forget) - for high-throughput scenarios
 *   <li>Sync - wait for persistence, receive resulting events
 *   <li>Speculative - what-if execution without persistence
 * </ul>
 *
 * <p>Usage:
 *
...
public class CommandHandlerClient implements AutoCloseable {
  ...
  private final CommandHandlerCoordinatorServiceGrpc.CommandHandlerCoordinatorServiceBlockingStub
      stub;
   ...
   *
   * <p>Returns immediately after the coordinator accepts the command. The command is guaranteed to
   * be processed, but the client doesn't wait.
   *
   ...
   *
   * <p>Blocks until the command handler processes the command and events are persisted. The
   * response includes the resulting events.
   *
   ...
   *
   * <ul>
   *   <li>Form validation - "Will this order succeed?"
   *   <li>Preview - "What events would this produce?"
   *   <li>Testing - verify business logic without polluting event store
   * </ul>
   *
  ...
  private static String formatEndpoint(String endpoint) {
    // Audit #39: lenient UDS prefix detection — recognise absolute path,
    // ./relative, unix:..., unix:/..., unix:///... as UDS; everything
    // else treated as TCP. Mirrors Python `_create_channel` and
    // Rust `detect_uds_path`.
    var uds = Transport.detectUdsPath(endpoint);
    if (uds.isPresent()) {
      // Normalize to the gRPC `unix://...` form expected by Netty.
      String path = uds.get();
      if (path.startsWith("/")) {
        return "unix://" + path;
      }
