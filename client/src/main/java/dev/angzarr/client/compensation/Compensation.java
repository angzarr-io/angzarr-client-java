 ...
 *
 * <p>Provides convenient factory methods for creating compensation responses in aggregates and
 * process managers when handling saga/PM rejections.
 *
...
public final class Compensation {
   ...
   *
   * <p>Use when the aggregate doesn't have custom compensation logic for a saga. The framework will
   ...
   *
   * <p>Use when the aggregate emits events to record compensation. The framework will persist these
   * events and NOT emit a system event.
   *
  ...
  public static BusinessResponse emitCompensationEvents(EventBook eventBook) {
    return BusinessResponse.newBuilder().setEvents(eventBook).build();
  }
