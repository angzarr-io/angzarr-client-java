...
final class Dispatch {
  ...
  /**
   * Return the type URL's full-name suffix, or empty string if not a {@code type.googleapis.com}
   * URL.
   */
   ...
   *
   * <p>Audit #87 (Python audit #74 renumbered): decode failures surface as {@link
   * Status.Code#INVALID_ARGUMENT} with the stable {@link
   * dev.angzarr.client.error_codes.Codes#ANY_DECODE_FAILED} code and structured details ({@code
   * type_url}, {@code cause}) so downstream tooling can route on a stable identifier instead of
   * substring-matching the message. Mirrors Python's {@code _parse_any} / {@code _unpack_any}
   * helpers (dispatch.py audit #87 block) and Rust's {@code
   * ClientError::invalid_argument(codes::ANY_DECODE_FAILED, ...)}.
   */
  static Message decodeMessage(Any payload, Class<?> messageClass) {
    try {
      ...
      throw new DispatchException(
          ...
          dev.angzarr.client.error_codes.Messages.ANY_DECODE_FAILED,
          dev.angzarr.client.error_codes.Codes.ANY_DECODE_FAILED,
          java.util.Map.of(
              dev.angzarr.client.error_codes.Keys.TYPE_URL,
              payload.getTypeUrl(),
              dev.angzarr.client.error_codes.Keys.CAUSE,
  ...
  /**
   * Pack a handler return value (single {@link Message}, {@link EventBook}, {@code Iterable}, or
   * {@code null}) into an {@link EventBook} starting at {@code baseSeq}.
   */
  ...
  static String fullNameOf(Class<?> messageClass) {
    try {
      ...
      throw new DispatchException(
          Status.Code.INTERNAL, "cannot derive full name for " + messageClass.getName(), roe);
    }
  }
