 ...
 * dispatch and reused across every event in the book (so implementations can batch writes). Handler
 * methods are {@code @Handles(EventClass)} → {@code void (EventClass)} — return values are ignored.
 */
