 ...
 * Returned {@link SagaHandlerResponse}s are merged into a single {@link SagaResponse}.
 */
public final class SagaRouter implements Built {
  ...
  public List<String> outputDomains() {
    // Audit #42 / parity P3.2: deduped target domains from every saga's
    // @Saga(target=...). Mirrors Python `BuiltRouter.output_domains()`
    // and Rust `SagaRouter::output_domains`.
    java.util.LinkedHashSet<String> seen = new java.util.LinkedHashSet<>();
    for (Registration<?> r : registrations) {
      Saga s = (Saga) r.metadata().kindAnnotation();
      String t = s.target();
      if (t != null && !t.isEmpty()) {
        seen.add(t);
      }
  ...
  /**
   * Audit #74: deduped subset of {@link #outputDomains()} where the saga declared {@code @Saga(sync
   * = true)}. The server uses this to wire one {@code OutputDomainProbe} per sync target;
   * everything else rides the {@code BusProbe} (if any). Mirrors Python's {@code
   * SagaRouter.sync_output_domains()} and Rust's {@code SagaRouter::sync_output_domains}.
   */
  ...
  /**
   * Audit #74: per-handler check — true when at least one registered saga declares an async target
   * (i.e. {@code sync = false}). Cannot be derived as {@code outputDomains() - syncOutputDomains()}
   * because two sagas may share the same target with different sync flags; the per-handler view
   * correctly reports "has async outputs" in that mixed case.
   */
      ...
      for (Map.Entry<Class<?>, MethodHandle> entry : r.metadata().handles().entrySet()) {
        ...
        try {
          ...
          throw new DispatchException(
              ...
              "@Handles on saga " + r.handlerClass().getSimpleName() + " failed: " + t.getMessage(),
              t);
        }
  ...
  /**
   * Pick the second argument based on the handler method's declared parameter type. Accepts {@link
   * SagaContext} (for handlers that need the source book / source root) or {@link Destinations}
   * (for pure stamping handlers).
   */
