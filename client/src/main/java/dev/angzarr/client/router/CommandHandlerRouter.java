...
public final class CommandHandlerRouter<S> implements Built {
  ...
  public List<String> outputDomains() {
    // Command handlers emit events, not cross-domain commands. Always empty.
   ...
   * {@code @Applies} methods. Useful for replay-style RPCs that need to materialize state without
  ...
  /**
   * Dispatch a {@link ContextualCommand} to all registered handlers matching {@code (domain,
   * type_url)}.
   *
   * <p>Multi-handler fan-out: every matching factory is invoked once, handlers run in registration
   * order, events are concatenated into one {@link EventBook}, and the sequence counter advances
   * monotonically across the merged stream. State is rebuilt independently per handler instance via
   * its own {@code @Applies} methods.
   */
  public BusinessResponse dispatch(ContextualCommand request) {
    ...
    if (!cmdBook.hasCover() || cmdBook.getCover().getDomain().isEmpty()) {
      throw new DispatchException(Status.Code.INVALID_ARGUMENT, "CommandBook cover has no domain");
    }
    ...
    // Derive the suffix from the proto descriptor so the cross-language
    // proto-package rename (`angzarr.*` → `angzarr_client.proto.angzarr.*`)
    // doesn't leave a stale string here.
    if (typeSuffix.equals(Notification.getDescriptor().getFullName())) {
  ...
  private BusinessResponse dispatchNotification(Any commandAny, ContextualCommand request) {
    ...
    if (notification.hasPayload()) {
      try {
        rejection = notification.getPayload().unpack(RejectionNotification.class);
      } catch (InvalidProtocolBufferException ipbe) {
        throw new DispatchException(
            Status.Code.INVALID_ARGUMENT,
    ...
    for (Registration<?> r : registrations) {
      ...
      try {
        ...
        throw new DispatchException(
            Status.Code.INTERNAL,
            "@Rejected on " + r.handlerClass().getSimpleName() + " failed: " + t.getMessage(),
            t);
      }
      EventBook packed = Dispatch.packEvents(emitted, currentSeq);
      for (EventPage page : packed.getPagesList()) {
