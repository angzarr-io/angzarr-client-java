 ...
 *
 * <p>Carries the full source {@link EventBook} that triggered this saga invocation plus a {@link
 * Destinations} view for stamping outbound commands. Mirrors Python's {@code SagaContext} in {@code
 * angzarr_client/saga_context.py}.
 *
 * <p>Saga handlers that only need sequence stamping can use {@link #destinations()} directly;
 * handlers that need the source aggregate's root, correlation id, or trigger event can read {@link
 * #source()} or the convenience accessors.
 */
public final class SagaContext {
  ...
  /**
   * Root UUID bytes of the source aggregate, or {@link ByteString#EMPTY} if no root is set. Useful
   * for correlating emitted facts back to the triggering aggregate.
   */
